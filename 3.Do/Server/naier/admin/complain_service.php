<?php 
	...
	if($action=="list"){
		$page = $_REQUEST["page"];
		$rows = $_REQUEST["rows"];
		$sql = "select * from complain_view  ";
		$result = mysql_query($sql,$con);
		$json = array();
  		$json["total"] =  mysql_num_rows($result);
  		$sql = $sql." order by status asc,complainid desc ";
		...
		$sql = "delete from complain where id=".$_REQUEST['id'];
		mysql_query($sql);
		echo "success";
	}else if($action=="edit"){
		$id = $_REQUEST["id"];		
		...
		$sql = "update complain set status='1' where id=".$_REQUEST['id'];
		mysql_query($sql);
		echo "success";
	}
?>
