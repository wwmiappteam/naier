...
function optFormater(value, rowData, rowIndex) {
	var optstr =  "";
	if(rowData.status=="0"){
		optstr = optstr+"<a onclick=chulidata("+rowData.id+"); class='icon icon_approval' title='处理' href='#'></a>";
	}
	optstr = optstr +"<a onclick=deletedata("+rowData.id+"); class='icon icon_delete' title='删除' href='#'></a>";
	return optstr;
}
